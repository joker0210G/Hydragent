...
| 5 | Subagent Swarm & Model Council | **Week 19 done; Weeks 20–22 not started** | `hydragent-planner` crate is real and working: `dag.rs` (cycle detection, topo sort), `decomposer.rs` (LLM-driven complexity classification + decomposition), `scheduler.rs` (`ReadyQueue`), `serializer.rs` (save/load), and `bin/planner_demo.rs` interactive demo. 5 unit tests in `tests/planner_tests.rs`. No `hydragent-swarm` crate, no Model Council, no SubAgent spawner, no mailbox, no self-healing replanner. |
